...
class Game {
  ...
  play(): void {
    ...
    for (let i = 0; i < this.blocks.length; ++i) {
      ...
      if (this.checkCollision(this.ball, this.blocks[i])) {
        ...
        (this.blocks[i].size.width = 0), (this.blocks[i].size.height = 0);
  ...
  checkCollision(ball: Ball, obj: any): boolean {
    const dX: number = Math.abs(ball.pos.x - obj.pos.x - (obj.size.width >> 1)),
      dY: number = Math.abs(ball.pos.y - obj.pos.y - (obj.size.height >> 1));
